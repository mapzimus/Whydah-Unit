...
| `bulk.html` | `/photo/bulk.html` | **Teacher bulk upload.** Dump a whole folder into the slideshow — no 3-photo limit. Captions default to file names; optional shared credit. Linked from the photo home as the first card. |
| `submit.html` | `/photo/submit.html` | **Showcase submission.** Each student gets three slots. Add a photo + caption per slot; **Replace**, **Update caption**, or **Delete** anytime if they uploaded the wrong one. Teacher panel at the bottom (also `#teacher`) uploads from the teacher’s computer on behalf of a student who can’t use a Chromebook. `#bulk` redirects to `bulk.html`. |
...
- Showcase submissions live entirely in storage under `showcase/`: each photo is a JPEG plus a
  ...
  `slot`, `created_at`, optional `deleted`). The sidecar uses an image MIME type because the
  `photos` bucket rejects non-image uploads — and rejects overwrite/delete from the public key —
  so replace/delete works by writing a *newer* sidecar for the same slot; the slideshow keeps
  only the latest non-deleted version per student slot. Bulk uploads set `bulk: true` +
  `entry_id` and skip the 3-slot limit. No extra database table is required.
